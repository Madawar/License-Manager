<div class=>
    ...
    @if ($license->license_type != '')
        ...
        <div class="w-auto m-8 text-gray-800 bg-white p-5 rounded-lg shadow-md sm:m-4">
            ...
            <div class="flex flex-col md:flex-row p-2 md:space-x-1 md:space-y-0 space-y-1 w-full">
                <div class="flex-auto">
                    <x-forms.input label="{{ Str::upper($license->license_type) }} Name"
                        placeholder="{{ Str::ucfirst($license->license_type) }} Name"
                        name="license.license_certification" wire:model="license.license_certification" />
                </div>
            ...
            <div class="flex flex-col md:flex-row p-2 md:space-x-1 md:space-y-0 space-y-1 w-full">
                ...
                <div class="flex-none">
                    <x-forms.select label="{{ Str::upper($license->license_type) }} Renewal Measure"
                        placeholder="Period" name="license.license_period"
                        :options="array('years'=>'years','months'=>'months')" wire:model="license.license_period" />
                </div>
            </div>
            <div class="flex flex-col md:flex-row p-2 md:space-x-1 md:space-y-0 space-y-1 w-full">
                <div class="flex-auto">
            ...
            <div class="flex flex-col md:flex-row p-2 md:space-x-1 md:space-y-0 space-y-1 w-full">
                <div class="flex-auto">
                    <x-forms.textarea label="Notes" placeholder="Notes" name="license.notes"
                        wire:model='license.notes' />
                </div>
            </div>
        </div>
    ...
    @endif
    <div class="flex flex-col md:flex-row p-2 md:space-x-1 md:space-y-0 space-y-1 w-full">
        <button class="btn btn-block" wire:click='saveLicense'>Save {{ Str::upper($license->license_type) }}</button>
    </div>
    <script>
